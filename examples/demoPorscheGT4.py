...
from pylgbst.hubSpecific import PorscheGT4
...
if __name__ == '__main__':
    ...
    try:
        #connection = connection_from_url(options.connection)  # get_connection_bleak(hub_name=TechnicMoveHub.DEFAULT_NAME)
        #connection = get_connection_bleak(hub_name="HUB NO.4")
        connection = get_connection_bleak(hub_name="Technic Move  ")
        parameters['connection'] = connection
    except ValueError as err:
        parser.error(err.args[0])

    #hub = TechnicMoveHub(hub_name="HUB NO.4", **parameters)
    #hub = TechnicMoveHub(**parameters)
    #hub = TrainSmartHub(**parameters)
    ...
    if hub is not None:
        try:
            ...
            #demo_led_colors(hub)
            #demo_voltage(hub)
            hub.led_color(COLOR_PURPLE)
            sleep(2)
            hub.led_color(COLOR_BLACK)
            hub.lightOn(100)
            sleep(2)
            hub.lightOn(50)
            sleep(2)
    ...
    #hub = PorscheGT4(hub_name="Technic Move")
    hub = PorscheGT4(**parameters)
    #hub = PorscheGT4()
    ...
    if hub is not None:
        try:
            ...
            hub.motorStop()
            sleep(2)
            hub.steeringWheelLeft(50)
            sleep(2)
            hub.steeringWheelLeft(100)
            sleep(2)
            hub.steeringWheelRight(50)
            sleep(2)
            hub.steeringWheelRight(100)
            sleep(2)
            hub.steeringWheelCenter()
            """
        finally:
            hub.disconnect()
